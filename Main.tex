...
\subsection{Worn fault sections: Implications on FMD}
...
\citet{Mignan2012} found that the FMD can be represented by asymmetric Laplace distribution and, more recently asymmetric Laplace mixture model (ALMM) \citet{Mignan2020}. While seismologists routinely rely on the scale-invariant nature of the Gutenberg-Richter law to extrapolate earthquake potentials above the magnitude of completeness ($M_{c}$), using the data below is typically not employed. Advantages proposed by ALMM is the use of the 'incomplete (scale-variant)' data and even provide specific statistical information regarding the heterogeneity in the mixture of seismicity. While \citet{Mignan2020} feels that power-law distributions are incomplete due to detection limitations, we feel this could also occur due to scale-variant physical processes such as the presence of wear-induced fault mirrors. Since this seismcity is `incomplete' it is not used in traditional statistical seismology. We feel that perhaps these scale-variant processes might be of importance since they appear to be critically linked to cascade-up behavior. This has also been proposed as an explanation to the complex FMDs \citep{Mignan2012} observed recently hydraulic shearing experiments of preexisting fractures in a crystalline rock formation (30 x 30 x 30 m$^{3}$) \citep{Villiger2019}.
...
Worn faults have been observed in nature as in the form of fault mirrors FMs but it is unclear how they truly evolve over time, their extent and how this evolution affects the frictional response.  In our wear-based model the reduction in heterogeneity caused a change in the behavior of a seismically prone asperities to change from a repeater- to foreshock-like and did so without a with an experimentally-driven distribution of heterogeneity.  Future experiments will need to investigate this behavioral evolution and updates on the stochastic descriptions of frictional faults that contain FMs may be necessary.

\acknowledgments
We would like to thank L. Villiger, B. Edwards T. Tormann, P. Bhattacharya and A. Mignan who provided important discussion points regarding the analysis. Key insights into simulations presented here are given by J.-P. Ampuero. T. Yamaguchi for the invitation to the Non-Linear Friction Workshop in Kyushu, JPN funded through the Science of Slow Earthquakes project (JSPS KAKENHI no. 2804). 
